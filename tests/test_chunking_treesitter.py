...
"""
    ...
    func_chunk = next((c for c in chunks if c["node_type"] == "function_definition"), None)
    ...
    # Check class chunk
    class_chunk = next((c for c in chunks if c["node_type"] == "class_definition"), None)
...
"""
    ...
    # Check function chunk
    func_chunk = next((c for c in chunks if c["node_type"] in ["function_definition", "function_declaration"]), None)
    assert func_chunk is not None
    assert func_chunk["language"] == "javascript"
    assert "greet" in func_chunk["text"]
    
    # Check class chunk
    class_chunk = next((c for c in chunks if c["node_type"] in ["class_definition", "class_declaration"]), None)
    assert class_chunk is not None
